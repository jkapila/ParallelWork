...
if __name__ == '__main__':
    ...
    parallelizer = pw.Parallelize(func=test_func, func_params={}, split_data=False, processes=-1)
    parallelizer.execute(arr)
    print('Results:')
    print(parallelizer.results())

    print('\nExecuting Parallel process with parameters to function with splits!')
    parallelizer = pw.Parallelize(func=test_func_withparam, func_params={'axis': 0}, processes=-1)
    parallelizer.execute(arr)
    print('Results:')
    print(parallelizer.results())

    print('\nExecuting Parallel process with parameters to function without splits!')
    parallelizer = pw.Parallelize(func=test_func_withparam, func_params={'axis': 0}, split_data=False, processes=-1)
    parallelizer.execute(arr)
    print('Results:')
    print(parallelizer.results())

    print('\nExecuting Parallel process with parameters to function with splits on Axis 1!')
    parallelizer = pw.Parallelize(func=test_func_withparam, func_params={'axis': 0}, split_data=True, split_axis=1,
                                  processes=4)
    parallelizer.execute(arr)
    print('Results:')
    print(parallelizer.results())
